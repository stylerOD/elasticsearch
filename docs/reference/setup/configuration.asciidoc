...
JVM settings or arguments. (but how?)
...
`256m`), and `ES_MAX_MEM` (defaults to `1g`). (but how?)

It is recommended to set the min and max memory to the same value, and
enable <<setup-configuration-memory,`mlockall`>>. (again, how?)
...
In order to test how many open files the process (which process?) can open, start it with
